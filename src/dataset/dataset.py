...
def load_specified_dataset(model_name, config):
    if model_name in ['MKMSR']:
        return MKMSRDataset
    if model_name in ['BERT4Rec']:
        return BERTDataset
    elif model_name in ['KERL', 'MELOD']:
        return KERLDataset
    elif model_name in ['CL4SRec', 'CL4SRec_Drop', 'CL4SRec_Rec', 'ICLRec', 'IOCRec'] and not config.do_pretraining:
        return CL4SRecDataset
    elif model_name in ['CL4SRec_Structure'] and not config.do_pretraining:
        return CL4SRec_Separated_Dataset
    elif model_name in ['CL4SRec_SeCo'] and not config.do_pretraining:
        return CL4SRec_SeCoDataset
    elif model_name in ['STGCR', 'STGCR_WHS']:
        return ReversedSeqDataset
    elif model_name in ['SASRec_Reorder', 'SASRec_Reorder_UCL']:
        return SeqWithReorderAugDataset
    elif model_name in ['URCL4SRec', 'URCL4SRec_NoNeg', 'MoCoSeCoRec']:
        return SeqWithReorderRepeatAugDataset
    elif model_name in ['SASRec_URCL']:
        return SeqWithRepeatAugDataset
    elif model_name in ['ContraRec']:
        return ContraRecDataset
    elif model_name in ['DuoRec']:
        return DuoRecDataset
    elif model_name in ['UniContraRec']:
        return UniContraDataset
    elif model_name in ['SASRec_Rec'] and not config.data_aug:
        return SequentialNoAugDataset
    elif model_name in ['KGSCL', 'KGSCL_Rec', 'KGSCL_Rec']:
        return KGSCLDataset
    elif model_name in ['SASRec_KGAUG']:
        return KGDataAugDataset
    elif model_name in ['SRGNN']:
        return SeqWithSessionGraphDataset
    else:
